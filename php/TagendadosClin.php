...
if (!$id_clinica) {
    die("Erro: clínica não encontrada na sessão.");
}

$sql = "SELECT id FROM clin_med WHERE id_clinica = ? AND situacao = 0";
...
while ($row = $result->fetch_assoc()) {
    ...
    $update->bind_param("i", $id_clin_med);
    $update->execute();
}
...
$sql = "SELECT a.id, a.dia, a.hora, a.status,
        ...
        WHERE cm.id_clinica = ?
        AND a.id_paciente <> 0
        AND a.status NOT IN ('concluido', 'negado', 'excluida')";
...
<?php
include("enviar_confirmacoes.php");
?>